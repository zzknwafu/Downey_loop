...
`[P0][backend][targets] 实现 PromptVersion / AgentVersion 领域模型`

### Body

#### 背景

当前产品已经确认不做重型 Prompt IDE，但必须支持轻量的被测对象管理，否则实验无法明确“正在测哪个 prompt 或 agent”。

#### 目标

实现 PromptVersion 与 AgentVersion 的领域模型，并支持实验通过 target 引用它们。
...
- 状态至少包括：
  ...
  - `FINISHED`
  - `FAILED`
- 支持与 Dataset / Evaluator / Target 关联
